...
/**
 * Class RBKCMigrationSourceDocumentsCSV
 *
 * Migrate the source CSV documents data to MySQL for lookup purposes.
 */
class RBKCMigrationSourceDocumentsCSV extends RBKCBaseMigration {
  public function __construct(array $arguments) {
    parent::__construct($arguments);

    $this->description = t('Import source document data from CSV to MySQL for lookup.');

    $source = drupal_get_path('module', 'rbkc_migration') . DIRECTORY_SEPARATOR . 'data' . DIRECTORY_SEPARATOR . 'RBKCDocuments.csv';
    $columns = array();
    $options = array(
      'enclosure' => '"',
      'delimiter' => ',',
    ...
    $source_key = array('documentid' => array('length' => 255, 'type' => 'varchar', 'not null' => TRUE, 'default' => ''));
